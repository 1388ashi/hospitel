@foreach($operations as $operation)
    <div class="modal fade mt-5" tabindex="-1" id="edit-menu-{{ $operation->id }}" role="dialog"
         aria-labelledby="modelTitleId" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="min-width: 50vw;">
                <div class="modal-header">
                    <p class="modal-title font-weight-bolder">ویرایش عمل</p>
                    ...
                    <form action="{{route('admin.operations.update', $operation->id)}}" method="POST">
                        ...
                        <div class="row">
                            <div class="col form-group">
                                <label class="font-weight-bold">نام عمل :</label><span class="text-danger">&starf;</span>
                                <input type="text" name="name" class="form-control" value="{{ $operation->name }}"
                                required/>
                            </div>
                            <div class="col form-group">
                                <label class="font-weight-bold">مبلغ به تومان :</label><span class="text-danger">&starf;</span>
                                <input type="text" name="price" class="form-control" value="{{ $operation->price }}"
                                required/>
                            </div>
                        </div>
